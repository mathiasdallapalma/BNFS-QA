...
def discretize(df, labels, n_bins, discretizer_strategy):
    ...
    discretizer = preprocessing.KBinsDiscretizer(
        n_bins=n_bins, encode='ordinal', strategy=discretizer_strategy)
    df[numerical_columns] = pd.DataFrame(discretizer.fit_transform(
        df[numerical_columns]), columns=numerical_columns, dtype=int)
