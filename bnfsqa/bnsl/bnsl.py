...
def main(config, df):
    ...
    if config["divide_et_impera"]:
        ...
        if strategy != "QA":
            log.error(
                "Divide et Impera approach can be used only with QA strategy.")
            exit(1)
        ...
        for df_i in dfs:
            ...
            bns += [bnsl_qa(examples, len(df_i.columns), states, strategy,
                            config["QA_kwargs"]["reads"], config["QA_kwargs"]["annealing_time"])]
        ...
        bn = bnsl_qa(examples, len(df.columns), states, strategy,
                     config["QA_kwargs"]["reads"], config["QA_kwargs"]["annealing_time"])

    elif strategy == "bnlearn":
        log.info("Retrieving BN structure using bnlearn library")
        model = bnlearn.structure_learning.fit(
            df, methodtype=config["bnlearn_kwagrs"]["search_algorithm"], scoretype=config["bnlearn_kwagrs"]["metric"])
        bn = model['adjmat']
...
def divide_et_impera(config, df):
    ...
    dfs: list of pd.DataFrame
        Splits of the discretized dataset
    """
    ...
    if mod < 2 and mod != 0:
        log.error(
            "Last group will be combosed by %d features, wich is impossible to solve." % mod)
        exit(1)
    elif mod/2 < n and mod != 0:
        log.warn(
            "Last group will be combosed by %d features. (Other group are composed by %d features)" % (mod, n))
...
def bnsl_qa(examples, n, states, method, nReads, annealTime):
    ...
    minXt, minY, readFound, occurrences, annealTimeRes = dwaveSolve(
        Q, indexQUBO, posOfIndex, label, method=method, nReads=nReads, annealTime=annealTime)

    narcs = n*(n-1)
    bn = torch.cat([torch.cat([torch.zeros(n-1).view(-1, 1), minXt[:narcs].view(-1, n)],
                   dim=-1).view(-1), torch.zeros(1)]).view(n, n).int()
    bn = bn.numpy()
    return bn
